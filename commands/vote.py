...
async def vote(ctx):
    ...
    if dataLength > 5:
        if str(datetime.now().date()) == data["Item"]["date"]:
            embed = discord.Embed(
                color=0x5965F3,
                title="🗳️  MEGABOT Voting",
                description="You've already voted today!",
            )
    ...
    embed = discord.Embed(
        color=0x5965F3,
        title="🗳️  MEGABOT Voting",
        description="[Click here](http://adfoc.us/82393897415395) to vote.",
    ).set_footer(text="Click 'skip' top-right!")
    await interaction.edit_original_response(embed=embed)
    ...
    while voted is False:
        ...
        if timePassed >= 120:
            embed = discord.Embed(
                color=0x5965F3,
                title="🗳️  MEGABOT Voting",
                description="You took too long to vote! Try again.",
            )
            await interaction.edit_original_response(embed=embed)
            return
    ...
    embed = discord.Embed(
        color=0x5965F3,
        title="🗳️  MEGABOT Voting",
        description="Vote registered! +50 <:MEGACOIN:1090620048621707324>",
    )
    await interaction.edit_original_response(embed=embed)
