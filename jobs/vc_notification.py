...
async def vc_notification(bot, member, before, after, cooldownUsersSet):
    ...
    if before.channel is None:
        logger.info(
            f" Voice-Channel-Notifcation: {member.id} added to activeUsersSet for 10 minutes."
        )
        ...
        timer = threading.Thread(
            target=vc_notif_set_remove, args=(cooldownUsersSet, member.id, 600)
        )
        timer.start()
        ...
        if role in member.roles:
            await channel.send(
                f"*MEGALORD* <@{member.id}> **joined** <#{after.channel.id}>. Alerting @everyone"
            )
        else:
